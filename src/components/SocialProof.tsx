    ...
    <section className="relative bg-dark-bg py-24 border-t border-white/5 font-primary overflow-hidden" id="social-proof">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 text-right">
        ...
        <div className="text-center">
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-y-6 gap-x-12 opacity-60 hover:opacity-100 transition-opacity duration-500">
            {TRUSTED_HOTELS.map((hotel, idx) => (
              ...
              >
                <span className="font-primary text-base sm:text-lg font-medium tracking-wider text-ivory/80">{hotel.name}</span>
                <span className="font-primary text-[9px] text-gold/80 mt-1 uppercase tracking-widest flex items-center gap-1.5">
                  <MapPin size={8} />
          ...
          <ThreeDCard className="min-h-[200px] hover:border-gold/30">
            <div className="p-8 text-right h-full flex flex-col justify-between">
              <div>
                ...
                <div className="mt-4 font-primary text-base sm:text-lg text-ivory/90 leading-tight font-semibold">
                  متوسط نمو الحجوزات المباشرة
                </div>
              </div>
              <p className="mt-3 text-xs text-ivory/50 leading-relaxed font-primary">
        ...
        >
          ...
          <div className="absolute top-6 right-6 text-gold/5 font-primary text-[120px] pointer-events-none select-none">“</div>
          
          <div className="relative z-10">
            <div className="flex items-center gap-1 text-gold mb-6 justify-start">
              <span className="font-primary text-xs text-ivory/40 ml-2">تقييم موثق 5.0</span>
              {[1, 2, 3, 4, 5].map((s) => (
                <Star key={s} size={14} fill="currentColor" />
              ))}
            </div>

            <blockquote className="font-primary text-lg md:text-2xl font-light leading-relaxed text-ivory/90">
            ...
            <div className="mt-8 flex items-center justify-between border-t border-white/5 pt-6 flex-wrap gap-4 flex-row-reverse">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-full border border-gold/30 bg-gold/10 flex items-center justify-center font-primary text-gold text-lg">
                  أ
                </div>
                <div className="text-right">
                  <div className="font-primary text-sm font-semibold text-white">عبد الرحمن الرشيد</div>
                  <div className="font-primary text-[9px] text-ivory/40 uppercase tracking-widest mt-0.5">
                    رئيس مجلس الإدارة، الرشيد للاستثمار ومحافظ الضيافة
                  </div>
                </div>
              </div>

              <div className="flex items-center gap-1.5 rounded-full bg-gold/5 border border-gold/20 px-3 py-1 font-primary text-[9px] uppercase tracking-widest text-gold font-semibold">
                <Award size={10} />
